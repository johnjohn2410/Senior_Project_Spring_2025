...
random test images from a directory, applies preprocessing transformations, and prepares them for model inference.
...
image_names = [img for img in os.listdir(TEST_IMAGE_DIRECTORY) if img.endswith(".jpg")]

# Randomly select test images
test_image_names = random.sample(image_names, 5)

# Load corresponding test images and true masks
test_image_list = [os.path.join(TEST_IMAGE_DIRECTORY, img) for img in test_image_names if img.endswith(".jpg")]
test_mask_list = [os.path.join(TEST_MASK_DIRECTORY, img.replace(".jpg", "_Segmentation.png")) for img in test_image_names]
